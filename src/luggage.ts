...
/**
 * Build a rounded-corner box without relying on three/addons.
 * Uses ExtrudeGeometry from a rounded-rect shape with a bevel.
 */
function roundedBoxGeometry(w: number, h: number, d: number, r: number): THREE.BufferGeometry {
  ...
  const bevel = Math.min(r * 0.6, d * 0.45);
  ...
  // After extrude with bevel: z extends from -bevel to (d - 2*bevel) + bevel = d - bevel.
  // Center span (d) at z=0.
...
function makeBackpack(color: number): THREE.Group {
  ...
  const pocket = new THREE.Mesh(
    roundedBoxGeometry(w * 0.7, h * 0.45, 0.12, 0.06),
